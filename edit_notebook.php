<?php
	...
	if (!empty($_POST)) {
		...
		$insertQuery = $dbh->prepare("UPDATE `igem_notebook` SET `title`=?, `text`=?, `date`=?, `team`=? WHERE id=?");
		...
		$select = $dbh->prepare('SELECT * FROM `igem_notebook` WHERE `id`=?');
		...
		<form role="form" method="post">
		  ...
		  <div class="form-group">
			...
			<select class="form-control" name="team">
			  <option value="Yeast" <?php if ($item['team'] == 'Yeast'): ?>selected<?php endif; ?>>Yeast</option>
			  <option value="Bacteria" <?php if ($item['team'] == 'Bacteria'): ?>selected<?php endif; ?>>Bacteria</option>
			  <option value="Microfluidics" <?php if ($item['team'] == 'Microfluidics'): ?>selected<?php endif; ?>>Microfluidics</option>
			  <option value="I.T" <?php if ($item['team'] == 'I.T'): ?>selected<?php endif; ?>>I.T</option>
			</select>
		  </div>
		  <div class="form-group">
		    <label for="text">Text (<a href="http://2014.igem.org/Special:Upload" target="_blank">Upload an image iGEM's server</a>)</label>
		    <textarea class="form-control" rows="20" name="text"><?php echo htmlspecialchars(stripslashes($item['text'])); ?></textarea>
		  </div>
